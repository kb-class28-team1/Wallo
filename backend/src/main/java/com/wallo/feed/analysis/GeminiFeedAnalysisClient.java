...
public class GeminiFeedAnalysisClient implements FeedAnalysisClient {
    ...
    private String buildPrompt(String spendingType, String category) {
        return """
                ...
                spendingType이 SPENT가 아닌데 첫 판단에서 0원이 나오면 바로 확정하지 말고,
                최종 JSON을 작성하기 전에 미디어를 처음부터 한 번 더 자세히 확인하세요.
                두 번째 확인에서도 앞의 근거 기준을 충족하는 가격·수량·할인·절약 행동이 보이지 않을 때만 0원을 확정하세요.
                두 번째 확인에서 유효한 근거를 찾으면 그 근거로 다시 계산한 금액을 사용하세요.
                두 번 확인한 뒤에도 분석할 근거가 없어서 0원을 확정하는 경우 summary에는
                반드시 "분석 불가: 미디어를 다시 확인했지만 절약 근거를 확인하지 못해 0원으로 처리했습니다."를 포함하세요.
