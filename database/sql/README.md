...
MySQL 서버 연결 → `create_database.sql` 실행 → IntelliJ Database 새로고침 → `wallo` 선택 → `news.sql` → `financial_term.sql` → `news_report.sql` → `news_term.sql` 순서로 실행 → 테이블 생성 확인
...
- `news_report`: `news_id`가 `news.news_id`를 참조(FK) → **`news.sql` 실행 후**에 실행해야 함
- `news_term`: `news_id`가 `news.news_id`를, `term_id`가 `financial_term.term_id`를 각각 참조(FK) → **`news.sql`, `financial_term.sql` 둘 다 실행한 후**에 실행해야 함
