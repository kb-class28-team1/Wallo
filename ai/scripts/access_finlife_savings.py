...
OUTPUT_FILE = AI_ROOT / "data" / "raw" / "finlife_saving_products.csv"

EXAMPLE_MONTHLY_PAYMENT = 100_000
...
def calculate_after_tax_rate(value: Any) -> str:
    ...
    return f"{rate * (1 - INTEREST_TAX_RATE):.2f}%"


def calculate_saving_example(
    ...
    principal = EXAMPLE_MONTHLY_PAYMENT * save_term
    maturity_amount = principal + after_tax_interest

    return {
        "세전 이자(예시)": f"{round(gross_interest):,.0f}",
        "세후 이자(예시)": f"{round(after_tax_interest):,.0f}",
        "만기 수령액(예시)": f"{round(maturity_amount):,.0f}",
    }
...
def create_product_row(
    ...
    example = calculate_saving_example(
    ...
    row = {
        ...
        "적립유형 코드": clean_text(option.get("rsrv_type")),
        "적립유형명": clean_text(option.get("rsrv_type_nm")),
        ...
        "월 납입액(예시)": f"{EXAMPLE_MONTHLY_PAYMENT:,}",
        "세전 이자(예시)": example["세전 이자(예시)"],
        "세후 이자(예시)": example["세후 이자(예시)"],
        "만기 수령액(예시)": example["만기 수령액(예시)"],
        ...
        "데이터 출처": "금융감독원 금융상품한눈에 적금 Open API",
        "수집일시": collected_at,
    }
...
def crawl_all_products(
    ...
    with requests.Session() as session:
        for group_no, group_name in TOP_FINANCIAL_GROUPS.items():
            ...
            print()
            print("=" * 80)
            print(f"{group_name} 적금 수집 시작 ({group_no})")
...
def main() -> None:
    try:
        ...
        print()
        print("=" * 80)
        print(f"전체 적금 옵션 {len(products)}개 수집 완료")
