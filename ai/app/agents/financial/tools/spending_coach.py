...
def _period(arguments: dict[str, Any], today: date) -> tuple[date, date, date, date, str]:
    kind = str(arguments.get("periodType", "MONTHLY")).upper()
    reference = date.fromisoformat(arguments.get("referenceDate") or today.isoformat())
    if kind == "CUSTOM":
        ...
        return start, end, start - timedelta(days=days), start - timedelta(days=1), kind
    if kind == "WEEKLY":
        ...
        return start, end, comparison_start, comparison_start + (end - start), kind
    start = reference.replace(day=1)
    next_month = (start.replace(day=28) + timedelta(days=4)).replace(day=1)
    month_end = next_month - timedelta(days=1)
    end = today if start <= today <= month_end else month_end
    previous_end = start - timedelta(days=1)
    return start, end, previous_end.replace(day=1), previous_end, "MONTHLY"
...
def _repeating_patterns(items: list[ConsumptionTransaction], end: date) -> dict[str, Any]:
    ...
    recent = _in(items, recent_start, end)
    ...
    overall_daily = sum(weekday_amounts.values()) / 28 if recent else 0
    weekday_signals = []
    for day in range(7):
        repeated = sum(1 for (week, wd), amount in week_day_amounts.items()
                       if wd == day and amount >= overall_daily * 1.3)
        if repeated >= 3 and weekday_counts[day] >= 3:
            weekday_signals.append({"weekday": day, "weeks": repeated, "transactionCount": weekday_counts[day]})
    ...
    for slot in slots:
        ...
        repeated = sum(1 for week, total in weekly_totals.items()
                       if total and weekly_slots[(week, slot)] / total >= .4)
...
def analyze(context: ConsumptionContext, arguments: dict[str, Any], today: date | None = None) -> dict[str, Any]:
    ...
    start, end, previous_start, previous_end, kind = _period(arguments, today)
    ...
    sufficient = len(current) >= 3 and current_total >= 30_000
    base = {"periodType": kind, "analysisPeriod": {"start": str(start), "end": str(end)},
    ...
    if not sufficient:
        base["message"] = "아직 소비패턴을 판단할 수 있는 거래가 충분하지 않아요. 내역이 조금 더 쌓이면 분석해드릴게요."
        return base
    ...
    budget = None
    if context.budget and kind == "MONTHLY" and context.budget.target_month == start.strftime("%Y-%m"):
        usage = current_total / context.budget.total_amount * 100
        ...
        budget = {"budgetAmount": context.budget.total_amount, "usageRatePercent": round(usage, 1),
                  "monthProgressPercent": round(progress, 1), "gapPercentagePoints": round(gap, 1), "status": status}
    base.update({"totalChange": total_change, "categorySurges": surges[:3], "newSpending": new_spending,
                 "oneOffHighSpending": one_off, "repeatingCategories": repeating_categories,
                 "budgetStatus": budget, "patterns": _repeating_patterns(context.transactions, end),
                 "recurringPaymentCandidates": _recurring([tx for tx in context.transactions if tx.date <= end]),
                 "positiveImprovements": improvements,
                 "continuousImprovement": {"twoConsecutiveMonthsDecreased":
                                             _two_month_improvement(context.transactions, end)}})
    return base
